# Contributing to Cortex for Obsidian

First off, thank you for considering contributing to Cortex for Obsidian! It's people like you who make Cortex for Obsidian such a great tool!
...
Unsure where to begin contributing to Cortex for Obsidian? You can start by looking through the `help-wanted` issues.

### Pull Requests

The process described here aims to:

- Maintain the quality of Cortex for Obsidian.
- Fix problems that are important to users.
- Engage the community in working towards the best possible Cortex for Obsidian.
- Enable a sustainable system for Cortex for Obsidian's maintainers to review contributions.
...
In the case of Cortex for Obsidian, you will need to:
...
If you work across multiple worktrees or just want one command to build and load the plugin into a test vault, use `pnpm run test:vault`. It runs `pnpm install`, builds, symlinks `main.js` / `manifest.json` / `styles.css` from the worktree into the vault's `.obsidian/plugins/cortex/` folder, and reloads the plugin in Obsidian via its CLI.
...
- Use the "Refresh index" button and see if it properly starts indexing. If it says "index is up-to-date", use "Clear Cortex index" and start indexing again (or equivalently, use "force re-index" command).
...
- **Discord**: [Join](https://discord.gg/bFtfKDQqZt) the server for Cortex dev discussions.
- **Email**: logan@brevilabs.com

Thank you for contributing to Cortex for Obsidian!
