...
Cortex Plus includes an **autonomous agent** that can reason step-by-step and decide which tools to use to answer your question. Instead of you specifying every step, the agent figures out what to do on its own.

This feature requires a [Cortex Plus](cortex-plus-and-self-host.md) license.
...
The agent activates automatically when you're in **Cortex Plus** mode. You don't need to do anything special — just ask your question.
...
- **Setting**: **Settings → Cortex → Plus → Autonomous Agent Max Iterations**
...
Cortex Plus has 13 built-in tools. Some are always active; others can be enabled or disabled.
...
Saves information to your memory when you explicitly ask the AI to remember something. See [Cortex Plus and Self-Host](cortex-plus-and-self-host.md#memory-system) for details.

> **Requires**: **Settings → Cortex → Plus → Reference Saved Memories** must be enabled. If this setting is off, the tool is not registered and memory commands will not work.

### Configurable Tools

These tools can be individually enabled or disabled in **Settings → Cortex → Plus → Tool Settings**:
...
- **Trigger**: Automatically for "create a note" requests, or explicitly with `@composer` (available in both Cortex Plus and Projects mode)
- **Behavior**: Shows a preview of the content before writing. You can review and accept or reject the change.
- **Auto-accept**: Enable **Settings → Cortex → Plus → Auto-accept edits** to skip the preview
...
Go to **Settings → Cortex → Plus → Tool Settings** to:
...
You can choose your preferred diff view in **Settings → Cortex → Plus → Diff View Mode**.
...
If you trust the agent and don't want to review every file change, enable **Auto-accept edits** in **Settings → Cortex → Plus**. File changes will be applied immediately without a confirmation step.

---

## Related

- [Cortex Plus and Self-Host](cortex-plus-and-self-host.md) — Licensing and memory
